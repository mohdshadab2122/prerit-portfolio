  ...
  const fetchAllData = async () => {
    try {
      const [
        ...
        fetch(
          "https://script.google.com/macros/s/AKfycbx_4dB3V7MHMCxNWSmFy_oIzHKC9bSsfxQfhdKymk3v-fkudJq_QC7FedHd_bgTsj2R/exec?page=home",
        ),
        fetch(
          "https://script.google.com/macros/s/AKfycbx_4dB3V7MHMCxNWSmFy_oIzHKC9bSsfxQfhdKymk3v-fkudJq_QC7FedHd_bgTsj2R/exec?page=awards",
        ),
        fetch(
          "https://script.google.com/macros/s/AKfycbx_4dB3V7MHMCxNWSmFy_oIzHKC9bSsfxQfhdKymk3v-fkudJq_QC7FedHd_bgTsj2R/exec?page=recognitions",
        ),
        fetch(
        ...
        fetch(
          "https://script.google.com/macros/s/AKfycbx_4dB3V7MHMCxNWSmFy_oIzHKC9bSsfxQfhdKymk3v-fkudJq_QC7FedHd_bgTsj2R/exec?page=publication_conferences",
        ),
        fetch(
          "https://script.google.com/macros/s/AKfycbx_4dB3V7MHMCxNWSmFy_oIzHKC9bSsfxQfhdKymk3v-fkudJq_QC7FedHd_bgTsj2R/exec?page=publication_journals",
        ),
        fetch(
          "https://script.google.com/macros/s/AKfycbx_4dB3V7MHMCxNWSmFy_oIzHKC9bSsfxQfhdKymk3v-fkudJq_QC7FedHd_bgTsj2R/exec?page=publication_preprints",
        ),
        fetch(
      ...
      const formattedDefensive = (
        defensiveJson.defensivePublications || []
      ).map((row: any) => ({
        title: row.title,
      ...
      const formattedTradeSecrets = (tradeJson.tradeSecrets || []).map(
        (row: any) => ({
          title: row.title,
          date: row.date,
          inventors: row.inventors
      ...
      const formattedEducation = (educationJson.education || []).map(
        (row: any) => ({
      ...
      const formattedPublications = (
        publicationJson.publicationConferences || []
      ).map((row: any) => ({
        title: row.title,
      ...
      const formattedHome = (homeJson.home || []).map((row: any) => ({
        ...
        // 🔗 SAFE LINKS
        links: {
          linkedin: Array.isArray(row.linkedin)
            ? row.linkedin[0]
            : row.linkedin || "",
          scholar: Array.isArray(row.scholar)
            ? row.scholar[0]
            : row.scholar || "",
          patents: Array.isArray(row.patents)
            ? row.patents[0]
            : row.patents || "",
        },
        // 🧱 CARDS CLEAN STRUCTURE
        cards: [row.card1 || "", row.card2 || "", row.card3 || ""].filter(
          Boolean,
        ),
